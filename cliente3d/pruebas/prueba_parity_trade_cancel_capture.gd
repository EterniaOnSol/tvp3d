...
# Captura QA-owned en vivo: CANCELACION DE UN COMERCIO ABIERTO (Phase 2G.1).
# Fixture `PARITY-TRADE-CANCEL-001`.
#
...
# ---------------------------------------------------------------------------
#
# Prueba que una cancelacion UNILATERAL termina el comercio para LOS DOS
# participantes y **no mueve ningun objeto**.
#
...
# ---------------------------------------------------------------------------
#
# Entrante `0x80` -> `Game::playerCloseTrade` (`game.cpp:3204-3212`), que
# simplemente llama a `internalCloseTrade(player)` con `sendCancel = true`.
#
...
# ---------------------------------------------------------------------------
#
# Cancela el participante que NO inicio el comercio. La funcion del servidor es
# simetrica, asi que cualquiera de los dos serviria; se elige el no-iniciador
# porque es el caso levemente mas fuerte -prueba que la cancelacion no es un
# privilegio de quien abrio- y porque deja explicito que el otro lado no hizo
# absolutamente nada.
#
# Cual de los dos cancela es **eleccion del harness** y NO entra al payload.
#
# ---------------------------------------------------------------------------
...
#       pruebas/prueba_parity_trade_cancel_capture.tscn
...
var _objeto_a := "leather boots"
var _objeto_b := "leather helmet"
...
var _nadie_acepto := true
var _cerro_para_el_que_cancela := false
var _cerro_para_el_otro := false
...
func _ready() -> void:
	print("=========================================================")
	print(" TVP3D QA - captura en vivo: cancelacion de comercio")
	print("=========================================================")
...
func _medir_base() -> void:
	_slot_a = _slot_por_nombre(_estado_a, _objeto_a)
	_slot_b = _slot_por_nombre(_estado_b, _objeto_b)
	if _slot_a <= 0 or _slot_b <= 0:
		if _espera > ESPERA_CORTA:
			_fallar("BLOCKED algun participante no tiene su objeto de prueba en una ranura de equipo")
		return
	_cid_a = int(_estado_a.inventario[_slot_a].get("cid", 0))
	_cid_b = int(_estado_b.inventario[_slot_b].get("cid", 0))
...
func _ofrecer_a() -> void:
	...
	if not _pidio:
		...
		_con_a.enviar_solicitar_comercio_inventario(_slot_a, _cid_a, id_b)
		return
	if _espera > 2.0:
		_pasar_a("ofrecer b")
...
func _ofrecer_b() -> void:
	...
	if not _pidio:
		...
		_con_b.enviar_solicitar_comercio_inventario(_slot_b, _cid_b, id_a)
		return
	if _espera > 2.0:
...
## Cancela UN SOLO participante: el que NO inicio. El otro no acepta, no cancela
## y no hace nada.
##
## Ninguno de los dos acepta en ningun momento. Es auditable: en todo el archivo
## NO hay ninguna LLAMADA a la aceptacion del transporte, es decir ninguna
## ocurrencia seguida de parentesis. La unica mencion del nombre esta en este
## comentario, que es texto y no codigo.
func _cancelar() -> void:
	if not _pidio:
		# Guarda PREVIA, y tiene que evaluarse SOLO antes de mandar la orden:
		# si se la deja arriba sin condicion, tambien corre en el frame
		# siguiente y entonces el cierre que produce nuestra propia
		# cancelacion exitosa se confunde con un cierre espontaneo.
		if _a_cerro or _b_cerro:
			_fallar("FAIL el comercio se cerro antes de que nadie cancelara")
			return
		_pidio = true
		print("B, que NO inicio el comercio, lo CANCELA. A no hace nada.")
		_con_b.enviar_cerrar_comercio()
		return
	if _espera > 1.0:
...
func _esperar_cierre() -> void:
	if _a_cerro and _b_cerro:
		_cerro_para_el_que_cancela = _b_cerro
		_cerro_para_el_otro = _a_cerro
		print("Una sola cancelacion cerro el comercio en LAS DOS sesiones.")
		_pasar_a("medir final")
		return
	if _espera > ESPERA_PASO:
		_fallar("FAIL el comercio no cerro en las dos sesiones tras la cancelacion: el que cancela=%s, el otro=%s"
			% [str(_b_cerro), str(_a_cerro)])
...
func _medir_final() -> void:
	...
	if _espera < 3.0:
		return
	var a_tiene_a := _posee(_estado_a, _id_mochila_a, _objeto_a)
	var a_tiene_b := _posee(_estado_a, _id_mochila_a, _objeto_b)
	var b_tiene_b := _posee(_estado_b, _id_mochila_b, _objeto_b)
	var b_tiene_a := _posee(_estado_b, _id_mochila_b, _objeto_a)
	print("Propiedad final: A conserva lo suyo=%s y NO tiene lo del otro=%s; B conserva lo suyo=%s y NO tiene lo del otro=%s"
	...
	if not _sin_transferencia:
		_fallar("FAIL la cancelacion movio algun objeto; cada participante debia conservar exactamente lo suyo")
		return
	if not _nadie_acepto:
		_fallar("FAIL alguien acepto; este fixture mide la cancelacion sin aceptacion")
		return
	print("CANCELACION LIMPIA: ningun objeto cambio de dueno.")
...
func _construir_observacion() -> void:
	_obs = {
		...
		"cancellation": {
			"no_participant_accepted": _nadie_acepto,
			"trade_closed_for_cancelling_participant": _cerro_para_el_que_cancela,
			"trade_closed_for_other_participant": _cerro_para_el_otro,
		},
		"no_transfer": {
			"offered_items_kept_original_owners": _sin_transferencia,
		},
	}
...
func _emitir_observacion() -> void:
	...
	print("Captura de cancelacion de comercio: OK")
	_terminar(0)
